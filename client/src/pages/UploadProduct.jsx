...
const [fieldName,setFieldName] = useState("")
  ...
  return (
    <section>
      ...
      <div className="grid p-4 h-full max-h-[calc(100vh-193px)] overflow-y-scroll scroll-bar ">
        <form className=" grid gap-3 " onSubmit={handleFormSubmit}>
                ...
                {data.image.map((imge, index) => {
                  return (
                    ...
                    >
                      ...
                      />
                      <div onClick={()=> handleDeleteImage(index)} className=" absolute bottom-0 right-0 p-1 bg-red-100 text-red-700 hover:bg-red-600 hover:text-white rounded-full cursor-pointer hidden group-hover:block">
                      <MdDelete />
                      </div>
                    </div>
                  );
          ...
          {/* category  */}
           <div>
            <label className=" font-medium"  htmlFor="">Catgory</label>
            <div>
               <select 
               ...
               >
                   <option  hidden>Select Category</option>
                   {
                    allCategory.map((c,index) => {
                      return (
                        <option value={c._id} key={index+"c.name"}>{c.name} </option>
                      )
                    })
                   }
               </select>
                ...
                data.category.map((c,index)=>{
                  return(
                    <div key={c._id+index+"c.name"} className=" text-sm flex items-center gap-2 shadow my-2 border border-primary-200 px-1 bg-blue-50">
                         <p>{c.name} </p>
                         <div 
                         onClick={()=> handleRemoveSelectedCategory(index)}
                         className="hover:text-red-600 cursor-pointer"><IoClose size={20}/></div>
                    </div>
                  )
                })
               }
           ...
           {/* subCategory */}
           <div>
            <label className=" font-medium"  htmlFor=""> Sub Catgory</label>
            <div>
               <select 
               className=" bg-blue-50 w-full p-2 rounded outline-none text-gray-500"
               name="category"
          ...
          {/* stock  */}
          <div className=" grid gap-1">
            <label className=" font-medium"  htmlFor="stock">No. of Stock</label>
            <input
              type="number"
          ...
          {/* price  */}
          <div className=" grid gap-1">
            <label className=" font-medium"  htmlFor="price">Product Price</label>
            <input
              type="number"
          ...
          <div onClick={()=> setOpenAddMoreDetails(true)} className=" hover:bg-primary-200 bg-white py-1 px-3 w-44 text-center font-semibold border border-primary-200 hover:text-black cursor-pointer rounded">
            Add more details
          </div>
           <button className=" bg-primary-100 hover:bg-primary-200 font-bold rounded w-full py-1">Submit</button>
      ...
      {/* add more field component  */}
        {
            openAddMoreDetails && (
             <AddMoreDetails close = {()=>setOpenAddMoreDetails(false)}
      ...
      <div>
        {
          fullImageView &&(
            <ViewFullImage url={fullImageView} close={()=>setFullImageView ("")}/>
          )
        }
      </div>
    </section>
